...
class HubertEncoderWrapper(FairseqEncoder):
    ...
    def forward(self, source, padding_mask, **kwargs):
        ...
        x, padding_mask, lw_align_logits, logits_a3_v0, logits_v0_a3, logits_v3_a0, logits_a0_v3 = self.w2v_model.extract_finetune(**w2v_args)
        ...
        return {
            ...
            "encoder_padding_mask": padding_mask,  # B x T
            "padding_mask": padding_mask
        }, lw_align_logits, {"logits_a3_v0": logits_a3_v0, "logits_v0_a3": logits_v0_a3, "logits_v3_a0": logits_v3_a0, "logits_a0_v3": logits_a0_v3}
...
class AVHubertSeq2Seq(FairseqEncoderDecoderModel):
    ...
    def forward(self, **kwargs):
        ft = self.freeze_finetune_updates <= self.num_updates
        with torch.no_grad() if not ft else contextlib.ExitStack():
            output, lw_align_logits, aux_output = self.encoder(**kwargs)
            # output = self.encoder(**kwargs)
        decoder_out = self.decoder(prev_output_tokens=kwargs['prev_output_tokens'], encoder_out=output)
        return decoder_out, lw_align_logits, aux_output
